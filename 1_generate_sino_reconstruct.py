...
def main():
    ...
    pattern = r"bro_(\d+)_(\d+)_(True|False)_(\d+)\.npy"
    # Find matching files
    files = glob.glob(os.path.join("phantoms", "bro_*.npy"))
    ...
    for f in files:
        ...
        if match:
            ...
            LUNG_NAME = f"phantoms/lung_{RESOLUTION}.npy"
            BRONCHI_NAME = f"phantoms/bro_{RESOLUTION}_{radius}_{fibre}_{n_axis}.npy"
            r = f"reconstructions/reconstruct_{RESOLUTION}_{radius}_{fibre}_{n_axis}.npy"
            if not os.path.exists(r):
                volume = make_volume(BRONCHI_NAME, LUNG_NAME)
