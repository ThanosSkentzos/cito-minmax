...
from utils import save_file, load_file, plot_lung_vista, plot_bronchi
...
LUNG_NAME = f"phantoms/lung_{RESOLUTION}"
BRONCHI_NAME = f"phantoms/bro_{RESOLUTION}_{RADIUS}_{FIBRE}_{N_AXIS}"
...
if not os.path.exists(f"{LUNG_NAME}.vtk"):
    lung_mesh = generate_lungs(volume)
    lung_mesh.save(f"{LUNG_NAME}.vtk")
else:
    lung_mesh = pv.read(f"{LUNG_NAME}.vtk")

if not os.path.exists(f"{LUNG_NAME}.npy"):
    implicit_function = vtk.vtkImplicitPolyDataDistance()
    implicit_function.SetInput(lung_mesh)
    xmin, xmax, ymin, ymax, zmin, zmax = lung_mesh.bounds
    for i in tqdm(range(int(xmin-1),int(xmax+2))):
        for j in range(int(ymin-1),int(ymax+2)):
            for k in range(int(zmin-1),int(zmax+2)):
                point = [i, j, k]
                if implicit_function.EvaluateFunction(point) >= 0:
                    volume[i, j, k] = 1
    lung_coords = np.argwhere(volume > 0.0)
    save_file(f"{LUNG_NAME}.npy", lung_coords)
else:
    lung_coords = load_file(f"{LUNG_NAME}.npy")
...
if not os.path.exists(f"{BRONCHI_NAME}.vtk"):
    bronchi = generate_lungs(volume)
    bronchi.save(f"{BRONCHI_NAME}.vtk")
else:
    bronchi = pv.read(f"{BRONCHI_NAME}.vtk")
